...
/// Streaming requests wait much longer for upstream response headers than
/// the old 120s: giant (~180K-token) Codex turns can legitimately sit in the
/// upstream queue for minutes, and cutting the wait early just makes Codex
/// re-send the same huge request in a retry loop. The client connection is
/// kept alive during the wait via SSE keepalive comments, so a long timeout
