...
开启后，生成的 model catalog 会在 `base_instructions` 中追加一段任务清单使用指引。该内容写在本地 `~/.codex/model-catalogs/<provider>.json` 明文文件中，用户可以随时查看和修改。首次配置与缺少 `planHints` 字段的旧配置默认开启；如需关闭可运行：

```bash
codex-gateway-lite set-plan-hints off
```

已有安装如果任务卡片没有出现，可运行：

```bash
codex-gateway-lite set-plan-hints on
```

agent 会检测配置变化并自动重写 model catalog，无需重跑 `init`。
